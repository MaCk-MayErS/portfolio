...
const header = document.getElementById('image-container');
const secretMenu = document.getElementById('image-container1');
const jPrixLink = document.getElementById('jPrix');
const aDessinLink = document.getElementById('aDessin');
...
// Vérifier si les éléments existent avant d'ajouter la logique de basculement
if (header && secretMenu) {
    // Fonction pour basculer entre le header et le secretMenu
    function toggleMenu() {
        // Afficher/cacher le header et le secretMenu correctement
        if (header.style.display === 'none' || header.style.display === '') {
            // Si le header est masqué ou n'a pas de style défini, on l'affiche et on cache le secretMenu
            header.style.display = 'flex';
            secretMenu.style.display = 'none';
        } else {
            // Si le header est affiché, on le cache et on montre le secretMenu
    ...
    jPrixLink.addEventListener('click', toggleMenu);
    aDessinLink.addEventListener('click', toggleMenu);
